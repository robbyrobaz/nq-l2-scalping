...
Fades moves to VAH (top) and VAL (bottom) of value area.
...
def find_signals(bars, df_trades, params=PARAMS):
    """Scan bars for value area rejection signals."""
    signals = []
    n = len(bars)
    va_pct = params['value_area_pct']
    boundary_ticks = params['boundary_touch_ticks'] * NQ_TICK_SIZE
    ...
    for i in range(n - 1):
        ...
        # SHORT at VAH
        if current_bar['high'] >= vah - boundary_ticks:
            signals.append({
                'bar_idx': i,
                'ts': current_bar['ts_utc'],
                'direction': 'short',
                'entry_price': min(current_bar['close'], vah + boundary_ticks),
                'vah': vah,
                'rejection_type': 'vah',
            })

        # LONG at VAL
        if current_bar['low'] <= val + boundary_ticks:
            signals.append({
                'bar_idx': i,
                'ts': current_bar['ts_utc'],
                'direction': 'long',
                'entry_price': max(current_bar['close'], val - boundary_ticks),
                'val': val,
                'rejection_type': 'val',
            })
...
def run(params=None):
    ...
    result = {
        'strategy_id': '013',
        'strategy_name': 'Value Area Rejection',
        'backtest_period': {
    ...
    out = Path(__file__).resolve().parents[2] / 'data' / 'results' / '013_2026-03-06.json'
